...
public class SeedController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<IActionResult> Import([FromHeader] string authorization)
    {
        ...
        var habitsFromDb = await _service.HabitService.GetAllHabitsAsync(userId, trackChanges: false);
        ...
        foreach (var habitFromBackup in habitsToAdd)
        {
            ...
            var addedHabit = await _service.HabitService.CreateHabitAsync(userId, habitDtoToAdd);
            var addedRepetitionCollection = await _service.RepetitionService.CreateRepetitionCollectionAsync(userId, addedHabit.Id, repetitionsToAdd, trackChanges: false);
